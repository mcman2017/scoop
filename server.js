...
/*	let myret;
	fs.readFileSync('database.yaml','utf8', (err, contents) => {
		console.log('Reading in', contents);
		if (err) { console.log ('Problem reading database.yaml'); }
		console.log(yaml.load( contents));
		myret = yaml.load(contents);
		}); */

   try {  
      var data = fs.readFileSync('database.yaml', 'utf8');
      console.log('Reading in', data);    
      let myret = yaml.load(data);
      return myret;
   }  catch(e) {
...
function saveDatabase() {
	console.log(yaml.dump(database));
	fs.writeFileSync('database.yaml', yaml.dump(database), (err) => {
		 if (err) { console.log ('Problem writing above to database.yaml'); }
		console.log('Sussess:  database.yaml writen with the above');
	});
}
...
//  const requestComment = request.body;
  ...
  if (requestComment && requestComment.body && requestComment.articleId && database.articles[requestComment.articleId] &&
    ...
    //console.log('and requestComment are', requestComment);
